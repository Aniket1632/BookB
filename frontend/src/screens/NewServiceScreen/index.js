...
const ServiceScreen = ({ history }) => {
	...
	const [originalServiceListArr, setOriginalServiceListArr] = useState([]);
	const pageLimit = 20;
	...
	const onSearchHandler = (event) => {
		...
		if (searchTerm.trim() === '') {
			setServiceListArr(originalServiceListArr);
			setPageNumber(1);
		} else {
			setPageNumber(1);
			const filteredServices = serviceListArr.filter((category) => {
				const categoryTitle = (category.category.title || '').toLowerCase();
				const filteredSubServices = category.subService.filter((service) =>
					(service.title || '').toLowerCase().includes(searchTerm)
				);
				return categoryTitle.includes(searchTerm) || filteredSubServices.length > 0;
			});
			setServiceListArr(filteredServices);
		}
			...
			{serviceList && serviceList.loading ? (
				...
				<div>
					{serviceList &&
						...
						serviceList.categories.data.result.length > 0 ? (
						<div className={Styles.tableContainer} style={{ height: '80vh' }}>
																			...
																			<Droppable droppableId="ChildTable">
																				{provided => (
																					...
																					<table {...provided.droppableProps} ref={provided.innerRef}
																						...
																						<thead>
																							<tr className={Styles.accordianContainer}>
